...
#define RXp2 16
#define TXp2 17
#define IR_FILTER_WINDOW_SIZE 3
...
const unsigned long dimInterval = 1000;  // Dim every 5000 milliseconds (5 seconds)
...
const int sensorPins[NUM_PODS] = {35, 39, 27, 34, 36, 33}; // IR pins 
const int ledPins[NUM_PODS] = {32, 23, 22, 18, 21, 4};  // LED pins
...
void setupPods() {
  ...
  // Set up Pod 4
  FastLED.addLeds<NEOPIXEL, 19>(pods[3].leds, NUM_LEDS_PER_POD);
  fill_solid(pods[3].leds, NUM_LEDS_PER_POD, CRGB::Black);
  pods[3].sensorPin = sensorPins[3];
  pods[3].ledPin = 18;

  // Set up Pod 5
  FastLED.addLeds<NEOPIXEL, 18>(pods[4].leds, NUM_LEDS_PER_POD);
  fill_solid(pods[4].leds, NUM_LEDS_PER_POD, CRGB::Black);
  pods[4].sensorPin = sensorPins[4];
  pods[4].ledPin = 18;

  // Set up Pod 6
  FastLED.addLeds<NEOPIXEL, 4>(pods[5].leds, NUM_LEDS_PER_POD);
...
CRGB fireColors[] = {CRGB::Red, CRGB::Maroon,  CRGB::DarkRed, CRGB::DarkRed, CRGB::Maroon, CRGB_Burgundy, CRGB::DarkRed, CRGB::Maroon,
                     CRGB::Red, CRGB::DarkRed, CRGB_Burgundy, CRGB::DarkRed, CRGB::Maroon, CRGB_Burgundy, CRGB::DarkRed, CRGB::Maroon};
...
CRGBPalette16 firePalette = CRGBPalette16(CRGB::DarkRed, CRGB_Burgundy, CRGB::DarkRed, CRGB::DarkRed, CRGB::Maroon, CRGB::DarkRed, CRGB::DarkRed, CRGB::Maroon, CRGB::Red, CRGB::DarkRed, CRGB::DarkRed,
                                          CRGB::DarkRed, CRGB::Maroon, CRGB_Burgundy, CRGB::DarkRed, CRGB::Maroon);
...
CRGBPalette16 westVirginiaPalette = CRGBPalette16(CRGB::Navy, CRGB::DarkBlue, CRGB::Blue, CRGB::Gold, CRGB::Gold, CRGB::Yellow, CRGB::Gold, CRGB::DarkOrange, CRGB::DarkBlue, CRGB::Navy,
                                                  CRGB::DarkBlue, CRGB::Blue, CRGB::Gold, CRGB::Yellow, CRGB::Gold, CRGB::DarkOrange);
...
void handleWestVirginiaCommand(const String& command, bool isLeft) {
  ...
  for (int podIndex = 3; podIndex < 6; ++podIndex) {  
...
void handleBeerBall() { fill_solid(pods[4].leds, NUM_LEDS_PER_POD, CRGB::Red); }
...
void processLEDCommand(String command) {
  ...
  for (unsigned int i = 0; i < command.length(); ++i) {
    if (command[i] == 'P' && i + 1 < command.length()) {
      ...
      switch (colorCode) {
        case 'L':
          color = CHSV(rainbowHue, 255, 255);
          rainbowHue = (rainbowHue + 10) % 256;
          break;
        case 'G':
          color = CRGB::Green;
          break;
...
void sensorTask(void* pvParameters) {
  ...
  int irSensorValues[IR_FILTER_WINDOW_SIZE] = {0};  // Array to store the last few IR sensor readings
...
void setup() {
  Serial.begin(9600);
  MySerial2.begin(31250, SERIAL_8N1, RXp2, TXp2);
...
// What you just witnesses above is eqivlant to trying to juggle while a dog chases you
